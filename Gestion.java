...
public class Gestion{
	...
	Test test2 = new Test(cob2);
	Test test1 = new Test(cob1);
	lesTests.addElement(test1);
	lesTests.addElement(test2);
	...
	while (true){
	    ...
	    case 5 : ajouterExperience(lesCobayes, lesTests, lesAnimaux); break;
	    case 6 : afficherExperience(lesCobayes, lesTests); break;
	    }
	}
    }
    ...
    */
    ...
    public static void ajouterExperience (Vector lesCobayes, Vector lesTests, Vector lesAnimaux)
    /*
    ...
    */
	...
	if (rep == 1){
		...
		for (Enumeration e2 = lesCobayes.elements(); e2.hasMoreElements();){ // parcourt vecteur contenant cobayes
			...
			path.ajouterLabyrinthe(lesCobayes, lesTests, item);
			return;
		    }
		}
	    }	    
		...
		for (Enumeration e2 = lesCobayes.elements(); e2.hasMoreElements();){ // parcourt vecteur contenant cobayes
		    ...
		    if (item instanceof Souris && item.getNumOrdre() == choix && item.getId() != item2.getId()){
			path.ajouterNourriture(lesCobayes, lesTests,item);
			return;
		    }
		}
	    }
		...
		for (Enumeration e2 = lesCobayes.elements(); e2.hasMoreElements();){ // parcourt vecteur contenant cobayes
			...
			path.ajouterImage(lesCobayes, lesTests, item);
			return;
		    }
		}
	    }
    ...
    public static void afficherExperience(Vector lesCobayes, Vector lesTests){
	/*
...
System.out.println("Quelle expérience voulez-vous afficher?\n 1 - Labyrinthe\n 2 - Nourriture\n 3 - Image\n 4 - Toutes");
            int rep = saisie_entier();
            switch(rep){
	    case 1 : path.afficherLabyrinthe(lesCobayes, lesTests); break;
	    case 2 : path.afficherNourriture(lesCobayes, lesTests); break;
	    case 3 : path.afficherImage(lesCobayes, lesTests); break;
	    default : 
		for (Enumeration e = lesCobayes.elements(); e.hasMoreElements();){
		    for (Enumeration e2 = lesTests.elements(); e2.hasMoreElements();){
			Cobaye cob = (Cobaye) e.nextElement();
			Test test = (Test) e2.nextElement();
			if (cob.getExp() instanceof Labyrinthe && cob.getId().equals(test.getId())){
			    test.afficheLabyrinthe();
			}
			else if(cob.getExp() instanceof Nourriture && cob.getId().equals(test.getId())){
			    test.afficheNourriture();
			}
			    else if(cob.getExp() instanceof Image && cob.getId().equals(test.getId())){
			    test.afficheImage();
			}
		    }
		}
